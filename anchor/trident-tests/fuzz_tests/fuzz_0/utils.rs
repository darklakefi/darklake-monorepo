...
pub static AMM_PARAMS: OnceLock<Mutex<AmmParameters>> = OnceLock::new();

pub fn get_token_x_decimals() -> u8
{
    let amm_params = AMM_PARAMS.get_or_init(|| Mutex::new(AmmParameters { token_x_decimals: 6, token_y_decimals: 6 }));  // Initialize if not already set
